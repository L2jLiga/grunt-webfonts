...
module.exports = function webFonts(grunt) {
  ...
  grunt.registerMultiTask('webfont', 'Compile separate SVG files to webfont', function () {
    ...
    // Source files
    let files = _.filter(this.filesSrc, isSvgFile);
    if (!files.length) {
    ...
    let previousHash = readHash(this.name, this.target);
    ...
    if (o.hash === previousHash) {
      ...
      let generatedFiles = wf.generatedFontFiles(o);
    ...
    function generateFont(done) {
      const engine = require('./engines/' + o.engine);
      engine(o, (result) => {
        if (result === false) {
    ...
    function generateStylesheets(done) {
      ...
      o.stylesheets.sort((a) => (a === 'css' ? 1 : -1)).forEach(generateStylesheet);
    ...
    function generateStylesheet(stylesheet) {
      ...
      // Generate CSS
      let ext = path.extname(o.template) || '.css';  // Use extension of o.template file if given, or default to .css
      o.cssTemplate = readTemplate(o.template, o.syntax, ext);
      let cssContext = _.extend(o, {
        iconsStyles: true,
        stylesheet,
      });
    ...
    function prepareHtmlTemplateContext() {
      ...
      const relativeRe = new RegExp(_.escapeRegExp(o.relativeFontPath), 'g');
      ...
      const htmlStyles = renderTemplate(o.cssTemplate, context);
      _.extend(context, {
        styles: htmlStyles,
      });
    ...
    function generateDemoHtml(done) {
      ...
      let context = prepareHtmlTemplateContext();

      // Generate HTML
      let demoTemplate = readTemplate(o.htmlDemoTemplate, 'demo', '.html');
      let demo = renderTemplate(demoTemplate, context);
    ...
    function embedFont(fontFile) {
      // Convert to data:uri
      let dataUri = fs.readFileSync(fontFile, 'base64');
      let type = path.extname(fontFile).substring(1);
      let fontUrl = 'data:application/x-font-' + type + ';charset=utf-8;base64,' + dataUri;
    ...
    function generateFontSrc(type, font, stylesheet) {
      let filename = template(o.fontFilename + font.ext, o);
    ...
    function renderTemplate(template, context) {
      try {
        let func = _.template(template.template);
        return func(context);
      }
      catch (e) {
    ...
    function getCssFilePath(stylesheet) {
      let cssFilePrefix = option(wf.cssFilePrefixes, stylesheet);
    ...
    function getDemoFilePath() {
      ...
      let name = o.htmlDemoFilename || o.fontBaseName;
    ...
    function saveHash(name, target, hash) {
      let filepath = getHashPath(name, target);
